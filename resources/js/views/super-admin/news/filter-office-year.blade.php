    ...
    <div class="row justify-content-start g-3">
        ...
        <div class="col-md-6">
            <div class="card modern-form-card">
                <div class="card-body">
                    <h4 class="card-title"><i class="mdi mdi-clipboard-text-outline icon-smd"></i> Rekap Berita -> Unit
                        Utama / Tahun</h4>

                    {{-- FORM EXPORT PDF --}}
                    <form action="{{ route('super-admin.news.export-yearly-report-filter') }}" method="GET"
                        id="exportFormYearly">
        ...
        <div class="col-md-6">
            <div class="card modern-list-card">
                <div class="card-body">
                    <h4 class="card-title"><i class="mdi mdi-format-list-bulleted-square icon-smd"></i> Daftar Berita</h4>
...
@push('scripts')
    <script>
        function fetchNewsYearly(page = 1) {
            ...
            if (!officeEl || !yearEl) return;

            let office = officeEl.value;
            let year = yearEl.value;

            fetch(`{{ route('super-admin.news.filter-office.yearly.list') }}?office=${office}&year=${year}&page=${page}`, {
                                        ...
                                        <div class="modern-news-card" data-id="${news.id}">
                                            <div class="news-content">
                                                <div class="news-image-container">
                                                    ...
                                                    <div class="news-source-badge">${limitText(news.office, 12)}</div>
                                                </div>
        ...
        document.getElementById('office').addEventListener('change', () => fetchNewsYearly());
