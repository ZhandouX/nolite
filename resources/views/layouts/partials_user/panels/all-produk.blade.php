<section id="panel-all" class="product-section tab-panel px-6 pt-4 py-0 md:px-10 md:py-10">
    ...
    @else
        {{-- PRODUK GRID --}}
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-2 sm:gap-8 md:gap-8 md:px-10">
                ...
                <div class="group bg-white rounded-lg overflow-hidden border border-gray-200 hover:border-gray-300 hover:shadow-lg transition-all duration-300 flex flex-col h-full"
                    ...
                    <div class="p-2 sm:p-2.5 flex flex-col gap-1 sm:gap-1.5 flex-grow">
                        ...
                        <a href="{{ route('produk.detail', $item->id) }}" class="block">
                            <h3
                                class="font-semibold montserrat text-left md:text-center font-semibold md:font-bold text-xs sm:text-sm md:text-sm lg:text-xl text-gray-800 group-hover:text-gray-900 line-clamp-2 leading-snug h-8 sm:h-10 overflow-hidden">
                                {{ $item->nama_produk }}
                            </h3>
                        </a>
                        ...
                        <div class="h-5 sm:h-6 flex items-center md:justify-center gap-1.5">
                            @if($adaDiskon)
                                <p class="text-left md:text-center text-xs sm:text-lg lg:text-lg md:text-lg font-bold text-red-900 truncate">
                                    Rp{{ number_format($hargaDiskon, 0, ',', '.') }}
                                </p>
                                <p class="text-left md:text-center text-[10px] sm:text-xs md:text-md text-gray-400 line-through truncate flex-shrink-0">
                                    Rp{{ number_format($item->harga, 0, ',', '.') }}
                                </p>
                            @else
                                <p class="text-left md:text-center text-xs sm:text-lg lg:text-lg md:text-lg font-bold text-gray-900 truncate">
                                    Rp{{ number_format($item->harga, 0, ',', '.') }}
                                </p>
                            @endif
                        </div>
