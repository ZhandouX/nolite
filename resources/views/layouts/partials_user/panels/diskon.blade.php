{{-- PANEL: DISKON --}}
<section id="panel-diskon" class="product-section tab-panel hidden px-6 pt-4 py-0 md:px-10 md:py-10">
    <div>
        @if($produkDiskon->isEmpty())
            <div class="flex flex-col items-center justify-center py-12 col-span-2 md:col-span-3">
                ...
                <p class="text-gray-500 text-lg text-center">Belum ada produk diskon.</p>
            </div>
        @else
            <div
                        ...
                        <div class="relative overflow-hidden bg-gray-50">
                            ...
                            @auth
                                <button type="button" onclick="toggleWishlist('{{ $item->id }}')"
                                    class="absolute bottom-2 right-2 w-8 h-8 md:w-12 md:h-12 flex items-center justify-center bg-black/40 backdrop-blur-sm rounded-full shadow hover:scale-110 transition duration-200">
                                    <svg id="heart-icon-{{ $item->id }}"
                                        class="w-5 h-5 md:w-8 md:h-8 text-gray-300 transition-colors duration-300 {{ in_array($item->id, $wishlistIds ?? []) ? 'text-red-500 fill-red-500' : 'text-gray-300 fill-none' }}"
                            ...
                            @else
                                <button type="button" onclick="openLoginModal()"
                                    class="absolute bottom-2 right-2 w-8 h-8 md:w-12 md:h-12 flex items-center justify-center bg-black/40 backdrop-blur-sm rounded-full shadow hover:scale-110 transition duration-200">
                                    <svg class="w-5 h-5 md:w-8 md:h-8 transition-colors duration-300 text-gray-300 fill-none"
                                        viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                                    </svg>
                        ...
                        <div class="p-2 sm:p-2.5 flex flex-col gap-1 sm:gap-1.5 flex-grow">
                            ...
                            <div class="flex h-5 sm:h-6 items-center md:justify-center gap-1.5">
                                @if ($item->has_diskon)
                                    ...
                                    <p class="text-xs md:text-sm text-gray-400 line-through flex-shrink-0 truncate">
                            ...
                            @php
                                ...
                                $averageRating = $item->ulasan->avg('rating') ?? 0;
                                $totalTerjual = $item->orderItems()
                            ...
                            <div class="flex items-center lg:justify-center gap-2 text-[10px] sm:text-xs text-gray-500">
                                ...
                                <span class="text-gray-600 text-right">
                                    Terjual {{ number_format($totalTerjual) }}
                                </span>
                            </div>
