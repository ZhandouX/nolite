            ...
            @foreach($produkDiskon as $item)
                <div class="group product-cards w-[300px] bg-white rounded-xl overflow-hidden border border-gray-300 shadow-sm">
                    ...
                    <div class="flex flex-col gap-0 md:gap-2">
                        <h3 class="text-sm md:text-xl font-bold text-gray-900 line-clamp-1 text-center">{{ $item->nama_produk }}
                        </h3>
                        ...
                        {{-- MODAL KERANJANG & BELI --}}
                        @include('layouts.partials_user.modal-cart')
                        @include('layouts.partials_user.modal-beli')
                    </div>
                </div>
            @endforeach
        </div>
