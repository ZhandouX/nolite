...
@section('content')
    <main class="container mx-auto px-4 py-4">
        ...
        <div class="grid lg:grid-cols-2 gap-6 mb-6">
            ...
            <div class="product-gallery">
                ...
                <div class="mb-4 fade-in">
                    <div class="main-image rounded-lg overflow-hidden bg-gray-100">
        ...
        <div class="section-card py-4 px-4 sm:p-6 mb-6 fade-in">
        ...
        <div class="flex flex-wrap gap-3 mb-8">
            <button
                class="filter-star px-4 py-3 bg-gradient-to-r from-gray-900 to-gray-800 text-white rounded-xl text-sm font-semibold transition-all duration-300 hover:from-gray-800 hover:to-gray-700 hover:shadow-lg hover:scale-105 active:scale-95 shadow-md"
                data-star="all">
            ...
            @for($i = 5; $i >= 1; $i--)
                <button
                    class="filter-star px-4 py-3 bg-white text-gray-700 rounded-xl text-sm font-semibold transition-all duration-300 hover:bg-gray-50 hover:shadow-lg hover:scale-105 active:scale-95 border border-gray-200 hover:border-gray-300 shadow-sm backdrop-blur-sm"
                    data-star="{{ $i }}">
        ...
        @if($produk->ulasan->isNotEmpty())
            <div id="ulasanContainer" class="space-y-2">
                @foreach($produk->ulasan as $u)
                    <div class="review-card fade-in bg-white rounded-2xl p-6 border border-gray-100 hover:border-gray-200 transition-all duration-500 hover:shadow-xl shadow-sm backdrop-blur-sm"
    ...
    <div class="fixed flex lg:hidden bottom-0 left-0 right-0 bg-white py-2 px-2 z-[70] gap-3 fade-in">
        <button type="button" onclick="toggleWishlistDetail('{{ $produk->id }}')"
        ...
        <button type="button" onclick="detailAddToCart('{{ $produk->id }}')" id="cartBtn-{{ $produk->id }}"
            class="flex-1 flex items-center justify-center border border-gray-700 text-gray-700 hover:bg-gray-600 hover:text-white gap-2 font-medium py-3 px-4 rounded-xl transition-all duration-200"
            {{ $produk->jumlah <= 0 ? 'disabled' : '' }}>
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
...
@push('script')
    ...
    <!-- <script>
        document.addEventListener('DOMContentLoaded', function () {
            window.syncCheckout = function (productId) {
                if ({{ $produk->jumlah }} <= 0) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Stok Habis',
                ...
                const checkoutBtn = document.getElementById(`checkoutBtn-${productId}`);
                const mobileCheckoutBtn = document.getElementById(`mobileCheckoutBtn-${productId}`);

                if (document.querySelector(`[data-item="${productId}"][data-color]`) && !color) {
                    showModernAlert('Pilih warna terlebih dahulu!')
                    return false;
                }

                if (document.querySelector(`[data-item="${productId}"][data-size]`) && !size) {
                    showModernAlert('Pilih ukuran terlebih dahulu!')
                    return false;
                }

                const checkoutQty = document.getElementById(`checkoutQty-${productId}`);
                const checkoutColor = document.getElementById(`checkoutColor-${productId}`);
                const checkoutSize = document.getElementById(`checkoutSize-${productId}`);

                const mobileCheckoutQty = document.getElementById(`mobileCheckoutQty-${productId}`);
                const mobileCheckoutColor = document.getElementById(`mobileCheckoutColor-${productId}`);
                const mobileCheckoutSize = document.getElementById(`mobileCheckoutSize-${productId}`);
