...
@section('content')
    <main class="container mx-auto px-4 py-6">
        ...
        <div class="grid lg:grid-cols-2 gap-8 mb-12">
                            ...
                            @foreach($produk->fotos as $key => $foto)
                                <img src="{{ asset('storage/' . $foto->foto) }}" alt="Thumbnail {{ $key + 1 }}"
                                    class="thumb w-16 h-16 object-cover rounded cursor-pointer border-2 transition-all duration-200
                                                                                    {{ $key === 0 ? 'border-gray-700' : 'border-gray-200 hover:border-gray-400' }}" onclick="setMainImage(this.src, this)" />
                            @endforeach
                        @else
            ...
            <!-- IMAGE MODAL -->
            <div id="imageModal"
                class="image-modal fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center hidden z-50 p-4">
                <div class="max-w-4xl max-h-full">
                    <div class="relative">
                        <button onclick="closeModal()"
                            class="absolute -top-12 right-0 text-white hover:text-gray-300 transition-colors p-2 rounded-full hover:bg-white hover:bg-opacity-10">
                ...
                <div class="section-card p-4 space-y-6 fade-in">
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($warnaList as $w)
                                    <button type="button" class="option-btn" data-color="{{ $w }}" data-item="{{ $produk->id }}"
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($produk->ukuran as $u)
                                    <button type="button" class="option-btn" data-size="{{ $u }}" data-item="{{ $produk->id }}"
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 w-fit">
                            <button type="button" onclick="decrementQty({{ $produk->id }})" class="qty-btn"
                            ...
                            <button type="button" onclick="incrementQty({{ $produk->id }})" class="qty-btn"
                        ...
                        <p class="text-xs text-gray-500 mt-2">Stok tersedia: <span
        ...
        {{-- CUSTOMER REVIEWS SECTION - COMPACT & RESPONSIVE --}}
        <div class="section-card p-4 sm:p-6 mb-6 fade-in">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                <div class="mb-4 md:mb-0">
                    <h2 class="text-xl sm:text-2xl font-bold text-gray-900 mb-1">Ulasan Pelanggan</h2>
                    <p class="text-gray-600 text-xs sm:text-sm">Pengalaman nyata dari pembeli produk ini</p>
                </div>

                {{-- RATING SUMMARY --}}
                <div class="flex items-center gap-4">
                    <div class="text-center bg-gray-50 rounded-xl px-4 py-3 border border-gray-100 min-w-[100px]">
                        <div class="text-xl sm:text-2xl font-bold text-gray-900 mb-1">{{ $avgRating }}</div>
                        ...
                        <p class="text-xs text-gray-500">{{ $totalUlasan }} ulasan</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2 mb-6 pb-4 border-b border-gray-100">
                <button
                    class="filter-star px-3 py-2 bg-gray-900 text-white rounded-lg text-xs font-medium transition-all duration-200 hover:bg-gray-800"
                    data-star="all">
                ...
                @for($i = 5; $i >= 1; $i--)
                    <button
                        class="filter-star px-3 py-2 bg-white text-gray-700 rounded-lg text-xs font-medium transition-all duration-200 hover:bg-gray-50 border border-gray-200 hover:border-gray-300"
                        data-star="{{ $i }}">
                        <span class="flex items-center gap-0.5">
                            @for($s = 1; $s <= $i; $s++)
                                <span class="text-yellow-400 text-xs">★</span>
                            @endfor
                            <span class="ml-0.5 font-semibold">{{ $i }}</span>
                        </span>
                    </button>
                @endfor
            </div>
            ...
            @if($produk->ulasan->isNotEmpty())
                <div id="ulasanContainer" class="space-y-3">
                    @foreach($produk->ulasan as $u)
                        <div class="review-card fade-in bg-white rounded-xl p-4 border border-gray-100 hover:border-gray-200 transition-all duration-300"
                            data-rating="{{ $u->rating }}">
                            <div class="flex items-start gap-3 mb-3">
                                <div
                                    class="w-10 h-10 bg-gray-500 rounded-full flex items-center justify-center text-white text-sm font-semibold flex-shrink-0">
                                    {{ substr($u->user->name ?? 'A', 0, 1) }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-2">
                                        <p class="font-semibold text-gray-900 text-sm truncate">{{ $u->user->name ?? 'Anonymous' }}
                                        </p>
                                        <span
                                            class="text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded-md whitespace-nowrap">{{ $u->created_at->format('d M Y') }}</span>
                                    </div>

                                    <div class="flex items-center mb-2">
                                        <div class="flex mr-2">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg class="w-3 h-3 {{ $i <= $u->rating ? 'text-yellow-400' : 'text-gray-300' }}"
                                    ...
                                    @if($u->orderItem)
                                        <div class="bg-gray-50 rounded-md px-2 py-1.5 mb-2">
                                            <p class="text-xs text-gray-600 truncate">
                                                <span class="font-semibold">Varian:</span>
                                                @if($u->orderItem->warna)
                                                    <span class="ml-1">{{ $u->orderItem->warna }}</span>
                                                @endif
                                                @if($u->orderItem->ukuran)
                                                    <span class="ml-1">• Size: {{ $u->orderItem->ukuran }}</span>
                                                @endif
                                            </p>
                                        </div>
                                    @endif
                            ...
                            <p class="text-gray-700 text-sm leading-relaxed mb-3 pl-13">{{ $u->komentar }}</p>

                            @if($u->fotos->isNotEmpty())
                                <div class="flex gap-2 overflow-x-auto pb-1 pl-13">
                                    @foreach($u->fotos as $f)
                                        <div class="relative group flex-shrink-0">
                                            <img src="{{ asset('storage/' . $f->foto) }}"
                                                class="w-14 h-14 object-cover rounded-lg cursor-pointer transition-all duration-300 group-hover:scale-105"
            ...
            @else
                <div class="text-center py-8 bg-gray-50 rounded-xl border border-gray-100">
                    <div class="w-16 h-16 mx-auto mb-3 bg-gray-100 rounded-xl flex items-center justify-center">
                        <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                        </svg>
                    </div>
                    <h3 class="text-base font-semibold text-gray-900 mb-1">Belum ada ulasan</h3>
                    <p class="text-gray-500 text-xs max-w-xs mx-auto">Jadilah yang pertama membagikan pengalaman menggunakan
                        produk ini</p>
                </div>
            @endif
        </div>
