...
<div class="container mx-auto px-4">
    ...
    <form method="GET" class="mb-4">
        <div class="flex">
            <input type="text" name="search" class="border p-2 flex-1" placeholder="Cari pengguna..." value="{{ request('search') }}">
        ...
        <tbody>
            ...
            <tr class="text-center">
                ...
                <td class="border p-2 space-x-1">
                    ...
                    {{-- Hapus --}}
                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus permanen pengguna ini?')">
                        @csrf @method('DELETE')
            ...
            {{-- Modal Tailwind --}}
            <div id="modal-{{ $user->id }}" class="fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50">
                <div class="bg-white p-6 rounded shadow-lg w-96 relative">
                    ...
                    <button onclick="closeModal('modal-{{ $user->id }}')" class="absolute top-2 right-2 text-gray-700 font-bold text-2xl">&times;</button>
                </div>
            </div>
            @empty
            <tr><td colspan="6" class="text-center">Tidak ada pengguna ditemukan</td></tr>
            @endforelse
        </tbody>
    </table>
