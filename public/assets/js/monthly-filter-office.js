function fetchNews() {
    ...
    if (!officeEl || !monthEl) return; // safety check

    let office = officeEl.value;
    let yearMonth = monthEl.value;

    fetch("{{ route('super-admin.news.filter-office.list') }}?office=" + office + "&yearMonth=" + yearMonth, {
...
document.getElementById('office').addEventListener('change', fetchNews);
